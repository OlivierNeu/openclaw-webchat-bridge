 ...
 * missing content, or a non-numeric baseUpdatedAtMs. `instanceName` (optional)
...
export function parseAgentFilesBody(raw: string): AgentFilesBody | null {
  ...
  if (obj.op === "get" || obj.op === "set") {
    ...
    if (obj.baseUpdatedAtMs !== undefined && typeof obj.baseUpdatedAtMs !== "number") {
      return null;
    }
    return {
      op: "set",
...
export async function performAgentFilesOp(
  ...
  // set — COMPARE-AND-SET (A4): re-get first; when the caller pinned a base
  // timestamp and the live file moved, refuse with 409 so a concurrent edit is
  // never silently overwritten. The pre-set content is returned as `before`
  // for the Convex-side audit/rollback record.
  ...
  if (body.baseUpdatedAtMs !== null && currentUpdatedAtMs !== body.baseUpdatedAtMs) {
    return {
      status: 409,
