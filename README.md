# openclaw-webchat-bridge

Node/TypeScript bridge for **openclaw-webchat**. It holds a persistent **operator
...
also needs a reachable Convex (run the openclaw-webchat app's `npx convex dev`).
...
```bash
docker build -t openclaw-webchat-bridge:dev .
```
